...
func UnlockVC(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	if common.ContainsString(settings.IgnoredChannelIDs, customVC.ID) {
		msg := "This channel is configured as ignored and cannot be modified."
		s.InteractionResponseEdit(i.Interaction, &discordgo.WebhookEdit{
			Content: &msg,
		})
		return
	...
	// Unlock permissions
	if err := setUnlockPerms(s, customVC.ID, i.GuildID, settings.VoiceExceptionRoleIDs); err != nil {
	...
	for _, t := range timestamps {
		if now.Sub(t) < RenameCooldownDuration {
...
func setUnlockPerms(s *discordgo.Session, channelID, guildID string, roleIDs []string) error {
